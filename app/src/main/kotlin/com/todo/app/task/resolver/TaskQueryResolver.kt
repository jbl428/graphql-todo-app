...
class TaskQueryResolver(
  ...
  @GraphQLName("todos")
  fun find(): List<Todo> =
    taskQueryService.find(userId).let { tasks -> tasks.map { Todo.by(it, userId) } }

  @GraphQLName("todo")
  fun findOne(todoId: Int): Todo? =
    taskQueryService.findOne(todoId.toLong(), userId).let { Todo.by(it, userId) }
}
